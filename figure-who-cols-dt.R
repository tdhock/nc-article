...
data.vec <- list(who="single", iris="multiple")
...
), by=.(output.value.columns, N.col, expr)]
...
gg <- ggplot()+
  ...
  facet_grid(. ~ output.value.columns, labeller=label_both)
dl <- directlabels::direct.label(gg, list(cex=0.75, "last.polygons"))
pdf("figure-who-cols-dt.pdf", 9, 3)
print(dl)
dev.off()
png("figure-who-cols-dt.png", 9, 3, units="in", res=100)
print(dl)
dev.off()
