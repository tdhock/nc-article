...
\item[Reshape] columns contain the data which is present in the same
  amount but in different shapes in the input and output. 
...
\item[Capture] columns are only present in the output, and contain
  substrings of the reshaped input column names which are captured
...
\begin{figure}
  ...
  \caption{The iris data set (W, black) is considered as the input to
    a wide-to-tall reshape operation. It has four columns to reshape,
    which are converted to either a single reshape column (S, blue) or
    multiple (2) reshape columns (M1, M2, red). Other output columns
    are either copied from the non-reshaped input data, or captured
    from the names of the reshaped input columns.}
