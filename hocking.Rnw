...
syntax, which requires specifying regex strings, group names, and
...
First note that \code{new.diag.list} is a sub-pattern variable that we
...
In the \pkg{nc} code above, an anonymous function is used to convert
the values captured in the \code{ymax} group to numeric (and it maps
the empty string to \code{Inf}). Such custom type conversion functions
are not supported by \pkg{tidyr}, so post-processing must be used:
...
(\code{ymax} appears four times). Note that numeric conversions that
do not require special logic (e.g. \code{ymin} above) can be specified
using \code{names\_ptypes} (but using \code{ymax=numeric()} above
results in a lossy cast error).
...
twenties. Assume we need an output data table with two reshape columns
(\code{age} and \code{gender}) as well as a capture column
(\code{group}). The \pkg{nc} syntax we would use is:
...
The \code{stats::reshape} function also suffers from the same
...
<<reshape>>=
stats::reshape(TC, 1:4, direction="long")
@

The result above incorrectly contains \code{treatment} and
\code{control} columns rather than the desired \code{age} and
\code{gender} columns; ages and genders are also mixed in the same
columns. To fix that we must rename the input column
names, putting the desired ouput reshape column names first:

<<reshapenames>>=
TC.renamed <- structure(TC, names=sub("(.*)[.](.*)", "\\2.\\1", names(TC)))
stats::reshape(TC.renamed, 1:4, direction="long")
@

However the result above still contains incorrect results in the
\code{gender} column. The correct result is can be obtained by sorting
the input column names:

<<reshapesort>>=
TC.sorted <- data.frame(TC.renamed)[, sort(names(TC.renamed))]
stats::reshape(TC.sorted, 1:4, direction="long")
@
...
The \pkg{cdata} package is extremely powerful, and can handle many
...
in \pkg{nc} to specify capture output columns, which are not output by
other functions such as \pkg{data.table::melt}. Input columns with
regular names do not need to be renamed/sorted for \pkg{nc} functions,
whereas renaming/sorting is necessary using other functions
(\code{data.table::melt}, \code{stats::reshape}). Verbose control
table code is always necessary with \pkg{cdata}, whereas a
simple/terse regular expression sytnax can often be used with \pkg{nc}
to simplify definition of reshape operations.
