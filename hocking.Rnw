...
\code{column} capture group becomes an output reshape column
...
\code{4554}). As with the iris data, there are a number of
interesting questions that can be answered by first reshaping the data
(e.g. which age groups and diagnosis types are the most frequent).
...
Above we define \code{new.diag.list} which we use as the first part of
the pattern in the call to \code{nc::capture\_melt\_single} above (and
we will use that sub-pattern again below). The result is a data table
in which all three capture columns (\code{diagnosis}, \code{gender},
\code{ages}) are character vectors, since no type conversion functions
were specified. If we want to extract two numeric columns from
\code{ages} (e.g. to use as interval-censored outputs in a survival
regression), we can provide a different pattern with type conversion
functions:
...
reshaping using regular expressions (\code{nc::capture\_melt\_single}
and \code{nc::capture\_melt\_multiple}). Both functions input a data
...
reshaped in the output to a single column with name defined by the
...
captured in the special \code{column} group. Values captured in other
groups are stored in capture columns in the output. Both functions
support arbitrary type conversion functions, which may be specified in
order to output numeric capture columns.
