...
detailed discussion of regular expression patterns, we refer the
...
regex engines, and we then discuss the new features involving data
table integration and data reshaping.
...
number of other arguments (\code{...}) which are used to define the
...
character string in parentheses, e.g. \code{(.*)} for both \code{part}
and \code{dim} arguments above. All of the sub-patterns are pasted
...
\code{nomatch.error = FALSE} argument is given because the default is
to stop with an error if any subjects do not match the specified
pattern (the fifth subject \code{Species} does not match). Under the
...
the default is \code{base::identity}, which simply returns the
...
this syntax makes it easy to parse non-tabular text output files from
...
\subsection{New \pkg{nc} functions for wide-to-tall data reshaping}
...
which involve the familiar iris data set. First, suppose we would like
...
W$\rightarrow$S). We can perform this operation using
\code{nc::capture\_melt\_single}, which inputs a data frame and a
pattern which should match the names of the input columns to
reshape. Any input columns with names that do not match the pattern
are considered copy columns; the output also contains a capture column
for each group specified in the pattern:
...
considered copy columns (\code{Species}). Each unique value captured
in the special \code{column} group becomes the name of an output
reshape column (\code{Petal}, \code{Sepal}); other groups are used to
create output capture columns (\code{dim}). These data can
be used to create the scatterplot using \pkg{ggplot2} via:
...
demonstrate that the new \pkg{nc} syntax is  more powerful and
less repetitive, without sacrificing speed.

\subsection{Comparison with \pkg{tidyr} for a single output reshape column}
...
sub-pattern variable
is a list which includes capture group names as well as a type
conversion function. These three parameters are specified as three
separate arguments in \pkg{tidyr}, which results in some separation
(e.g. group names defined on L5 but corresponding sub-patterns defined on L1)
and repetition (e.g. \code{gender} appears on L5 and L6) in the code. The
pattern also must be repeated: first in the \code{cols} argument (L4)
to specify the set of input reshape columns, second in the
\code{names\_pattern} argument (L7) to specify the conversion from
input reshape column names to output capture column values.
...
pattern is much more readable/composable using \pkg{nc} syntax,
...
\subsection{Comparison with \pkg{data.table} and \code{stats::reshape} for multiple reshape output columns}
...
regular order, \code{melt} returns an incorrect result. To get a
...
slightly slower than \pkg{data.table} (by only constant factors). In
our result figures we show the median and quartiles over 10 timings
using the \CRANpkg{microbenchmark} package on an Intel Core i7-8700
3.20GHz processor. We varied the number of rows/columns in each
experiment by copying/duplicating the rows/columns in each source data
set. The package versions that we used were:
...
variable number of rows, and the original number of columns (56). We first consider a pattern with three capture
...
(Figure~\ref{fig:timings-single-rows}, top). The slowest function was
\code{stats::reshape} (about 10 seconds for $10^5$ rows) and the
fastest function was \code{utils::stack} (about 30 milliseconds). The
other functions, including \code{nc::capture\_melt\_single}, showed
intermediate speeds (about 100 milliseconds to 1 second).
% stats.timings[`capture groups`==3 & N.rows==10^5, .(N.rows, expr, median)][order(-median)]
%    N.rows                     expr      median
% 1: 100000           stats::reshape 10.12547431
% 2: 100000  nc::capture_melt_single  1.05684602
% 3: 100000      tidyr::pivot_longer  0.77007870
% 4: 100000            tidyr::gather  0.24123151
% 5: 100000 cdata::unpivot_to_blocks  0.23222514
% 6: 100000           reshape2::melt  0.17593780
% 7: 100000         data.table::melt  0.13262595
% 8: 100000             utils::stack  0.02784099

We also tried using a regular expression
...
\code{nc::capture\_melt\_single} ($\text{mean}\pm\text{sd}$, $0.327\pm
0.106$ seconds for $10^5$ rows) is
about five times faster than \code{tidyr::pivot\_longer}
($1.774\pm 0.147$).
% > stats.timings[`capture groups`==5 & N.rows==10^5, .(N.rows, expr, mean, sd)][order(-mean)]
%    N.rows                    expr      mean        sd
% 1: 100000     tidyr::pivot_longer 1.7745367 0.1469365
% 2: 100000 nc::capture_melt_single 0.3272358 0.1055836
% > 1.77/0.32
% [1] 5.53125
% >
...
packages), and one experiment with converting the \code{day} capture
group to an integer (for which we only tested \pkg{tidyr} and
\pkg{nc}).
...
factors), and slightly faster than the other packages (\pkg{cdata},
\pkg{stats}, \pkg{tidyr})
...
(150). Again there are two desired reshape output columns, and we
...
we observed on the log-log plot
(Figure~\ref{fig:timings-multiple-cols}) that \pkg{nc} and
\pkg{data.table} have smaller asymptotic slopes (1.009--1.061) than the
other packages (1.805--2.232).
...
%    type conversions                      expr intercept    slope
% 1:                0 nc::capture_melt_multiple -4.451562 1.009440
% 2:                0          data.table::melt -4.931091 1.040888
% 3:                1 nc::capture_melt_multiple -4.742470 1.060737
% 4:                0  cdata::rowrecs_to_blocks -5.657154 1.805371
% 5:                0       tidyr::pivot_longer -7.384375 1.836068
% 6:                0            stats::reshape -6.321754 1.871556
% 7:                1       tidyr::pivot_longer -8.914466 2.232118
% >
These data suggest that the computation time of \pkg{nc} and
\pkg{data.table} is linear $O(C)$ for $C$ input reshape columns,
whereas the other packages are quadratic $O(C^2)$. In practice this
means that the speed of \pkg{nc} and \pkg{data.table} relative to the
other packages increases as the number of input reshape columns $C$
increases. For example with $C=4\times 10^5$ input reshape columns,
\pkg{nc} takes only about 15 seconds, wheras \pkg{tidyr} takes over an
hour.
% > stats.timings[
% + , .SD[N.col==max(N.col), .(
% +   N.col, pkg=sub("::.*", "", expr), median, minutes)][order(-minutes)],
% +   by=`type conversions`]
%    type conversions  N.col        pkg       median     minutes
% 1:                0  40005      cdata  490.6223806  8.17703968
% 2:                0  40005      stats  199.6798834  3.32799806
% 3:                0  40005      tidyr   11.1225393  0.18537565
% 4:                0  40005         nc    1.3393606  0.02232268
% 5:                0  40005 data.table    0.7054606  0.01175768
% 6:                1 400005      tidyr 3835.4010933 63.92335155
% 7:                1 400005         nc   14.5308902  0.24218150
% >
Therefore \pkg{nc} or \pkg{data.table} should be preferred
...
\pkg{nc} provides a syntax that reduces repetition in user code. When
...
type conversions. Also, our empirical timings suggest that \pkg{nc} is
faster when there are type conversions, or when there are many input
reshape columns and multiple output reshape columns.

We observed a surprising result in our empirical timings with multiple
reshape output columns and a variable number of input reshape columns
$C$. We expected that all of the packages would have similar
asymptotic timings (differing only in constant factors). However for
large $C$ we observed computation times in \pkg{nc} and its dependent package
\pkg{data.table} that were linear $O(C)$, which was much faster than the
other packages (\pkg{cdata},
\pkg{stats}, \pkg{tidyr}) which were quadratic $O(C^2)$. This result suggests that the speed of
these other packages could be improved by adopting the linear time
algorithm used in the \pkg{data.table} package.
...
using the efficient implementation in \code{data.table::dcast}. For
future work, we will be interested to explore other operations related to
machine learning and data visualization which could be simplified
using regular expressions.

\paragraph{Acknowledgements.} Thanks to Marc Tollis for providing the
example data output from the SweeD program.
