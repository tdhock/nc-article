...
\code{tidyr::pivot\_longer} & yes & unsorted & capture & yes & any & yes\\
\code{data.table::melt}, \code{measure} & yes & unsorted & capture & yes & any & yes\\
...
tidyr::pivot_longer(
  who, cols = grep(tidyr.who.range.pattern, names(who)),
...
column (e.g., \code{group} above). As a result of this research we have
proposed the following new feature for the \pkg{data.table} package,

<<dtmeasure>>=
melt(TC, measure.vars = measure(group, value.name, sep="."))
@ 
