...
output columns (Figure~\ref{fig:wide-to-tall}). We refer to the
columns involved in this conversion as ``data input columns'' and
``data output columns''; the output may include additional columns
with values captured from the input column names. For example the wide
iris data (W in Figure~\ref{fig:wide-to-tall}) have four data input
columns: \code{Sepal.Length}, \code{Sepal.Width}, \code{Petal.Length},
\code{Petal.Width}. For some purposes (e.g. displaying a histogram of
each data input column using facets in \CRANpkg{ggplot2}) the desired
reshaping operation results in a table with a single data output
column (S in Figure~\ref{fig:wide-to-tall}) and two columns captured
from input names. For other purposes (e.g. scatterplot to compare
Petal and Sepal sizes) the desired reshaping operation results in a
table with multiple data output columns (M1 in
Figure~\ref{fig:wide-to-tall}). We propose to use the new regular
expression syntax of \citet{HOCKING2019-namedCapture},
e.g. \code{part=".*",} \code{"[.]",} \code{dimension=".*"}, to define
both types of wide-to-tall data reshaping operations. In particular,
we propose using a single capturing regular expression for both (1)
defining the subset of data input columns to convert, and (2)
capturing additional output columns from the input column names. This
We will show that this results in a simple, powerful, non-repetitive
syntax for wide-to-tall data reshaping.

\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{figure-1-iris}
  \caption{Wide data input columns (W, black) can be reshaped to a
    single data output column (S, blue) or multiple data output
    columns (M1, M2, red). Rectangle background indicates column type
    (white for data, grey for columns with values that come from text
    captured from input column names). }
...
\begin{description}
\item[single] refers to support for converting input columns
  of the same type to a single output column.
\item[multiple] refers to support for converting input columns of
  possibly different types to multiple output columns; ``sorted''
  means that conversion works correctly only if the columns are sorted
  in a regular order, e.g. \code{Sepal.Length}, \code{Sepal.Width},
  \code{Petal.Length}, \code{Petal.Width}; ``unsorted'' means that
  conversion works correctly even if the columns are not sorted,
...
\item[regex] refers to support for regular expressions; ``match''
  ...
  means that the capture groups in the specified pattern are used to
  create new output columns, which is especially useful when the names
  consist of several distinct variables, e.g. \code{Sepal.Length};
...
Several C libraries providing regular expression engines are provided
...
stop with an error if any subjects do not match the specified
pattern. In order to see the pattern that is generated and passed to
the specified regex engine, we can use
