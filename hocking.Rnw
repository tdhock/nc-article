...
defining named capture groups in R code. Using this new syntax, the
...
libraries, (2) integration of regex and \pkg{data.table}
functionality, and (3) specifying wide-to-tall reshape operations with
a new syntax which results in less repetitive user code than other
packages.
...
proposed functions of the \pkg{nc} package. The third section provides
