...
reshaping packages. detailed timing measurements, in FOO

\begin{figure}
  \centering
  \includegraphics[width = \textwidth]{figure-who-both-rows.pdf}
  \caption{Timings for wide-to-tall data reshaping on an input data
    table with 56 columns to reshape and a variable number of rows (x
    axis). \textbf{Top} panel shows functions which provide some
...
\begin{figure}
  ...
  \caption{Timings for wide-to-tall data reshaping on an input data
    table with 7240 rows and a variable number of columns to
