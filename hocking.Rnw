...
\title{Regular expressions for wide-to-tall data reshaping using the
  \pkg{nc} package}
...
\code{data.table::melt} internally.
...
\section{New features for wide-to-tall data reshaping using regular expressions}
...
specifies the wide input data table to reshape. The next three
...
iris.tall[, `:=`(
  part = sub(iris.pattern, "\\1", variable),
...
The code above computes an equivalent result, but suffers from the
...
3.20GHz processor. Note that these timings include both the regex
matching (which should be relatively fast) and the data reshaping
operation (which should be relatively slow). We varied the number of
rows/columns in each experiment by copying/duplicating the
rows/columns in each source data set. The package versions that we
used were:
...
Future work TODO tidyr default when names\_pattern.... As a result of this research we have
