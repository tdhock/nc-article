...
\dfn{subject} string. For example, consider as subjects the column
names of the famous iris data set in R, \code{Sepal.Length}
\code{Petal.Width}, \code{Species}, etc. The pattern \code{.*[.].*}
matches zero or more non-newline characters, followed by a period,
followed by zero or more non-newline characters. It would match
\code{Sepal.Length} and
\code{Petal.Width} but it
would not match \code{Species}.
...
input columns to reshape. Any input columns with names that do not
match the pattern are considered copy columns; the output also
