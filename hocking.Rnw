...
\item[Reshape] columns contain the data which is present in the same
  amount but in different shapes in the input and output. These are
  referred to as \code{measure.vars} in the \code{melt} function in
  \pkg{data.table} and \pkg{reshape2}.
\item[Copy] columns contain data in the input which are each copied to
  multiple rows in the output. These are referred to as
  \code{id.vars} in the \code{melt} function.
